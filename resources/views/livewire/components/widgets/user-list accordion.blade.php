...
new #[Layout('layouts.iframe')] class extends Component
{
    ...
    public function accounts()
    {
        ...
        if($this->user->data['roles']['type'] == 'agency') {
            $accounts = $this->company->accounts->toArray();
            // $accounts = collection($this->company->accounts);
            // $accounts =  Account::where('company_id', $this->company->id)
            //     // ->with('users.status')
            //     ->get();
        }
        else if($this->user->accounts()->count() > 1) {
            $accounts = $this->user->accounts()
                ->get()
                ->toArray();
        }
        else {
            $accounts = $this->user->accounts()->with(['users.status'])
                ->get();

            $this->userList[$accounts[0]['ghl_id']] = $accounts[0]['users'];
        }


        logger('accounts calling');
    ...
    public function getUsersFromAccounts ($account_id)
    {
        // Check if users are already fetched for the account (key exists)
        if (isset($this->userList[$account_id])) {
            // unset($this->userList[$account_id]);
        ...
        $accounts = $this->company->accounts->where('ghl_id', $account_id)->first();
        $this->userList[$account_id] = $accounts->users;
    ...
    <div class="divide-y divide-gray-100 dark:divide-gray-700">
        @foreach ($this->accounts as $account)
            <div x-data="{open: false}"
                wire:key="{{ $account['ghl_id'] }}"
                >
                <div class="account--container"
                    data-id="{{ $account['ghl_id'] }}"
                    wire:click="getUsersFromAccounts('{{ $account['ghl_id'] }}')"
                    @click="open = !open"
                    >
                ...
                @if (isset($userList[$account['ghl_id']]))

                    @foreach ($userList[$account['ghl_id']] as $userData)
